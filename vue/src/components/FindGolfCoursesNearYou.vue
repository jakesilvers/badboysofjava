            ...
            <div class="card">
              ...
              <div class="card-body">
                ...
                <ul v-for="(course, index) in golfCourses" :key="index">
                  <li>
                    <strong>{{
                      course.poi ? course.poi.name : "Unknown"
                    }}</strong>
                  </li>
                  <li>{{ course.address.freeformAddress }}</li>
                  <li>
                    Distance: {{ (course.dist / 1609.34).toFixed(2) }} miles
                  </li>
                  <button
                    type="button"
...
export default {
  ...
  methods: {
    ...
    isCourseAdded(course) {
      return this.addedCourses.some(
        (c) => c.courseName === (course.poi ? course.poi.name : null)
      );
    },
    addCourse(course) {
      if (this.isCourseAdded(course)) {
        this.successMessage =
          "This course has already been added to your list of Golf Courses.";
        return;
      }
